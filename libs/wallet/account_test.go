...
func TestGetAccount(t *testing.T) {
	...
	// Retrieve account
	retrieved, _, err := MockWallet.GetAccount(wallet, account.Address)
	assert.Nil(t, err)
	assert.Equal(t, account.Address, retrieved.Address)

	// Retrieve adhoc account
	_, retrievedAdhoc, err := MockWallet.GetAccount(wallet, adhocAcct.Address)
	assert.Nil(t, err)
	assert.Equal(t, adhocAcct.Address, retrievedAdhoc.Address)

	// Retrieve unknown account
	_, _, err = MockWallet.GetAccount(wallet, "nano_1efa1gxbitary1urzix9h13nkzadtz71n3auyj7uztb8i4qbtipu8cxz61ee")
